...
if isa(q1,'qo') & isa(q2,'qo')
   ...
   elseif isequal(prod(q1.size),1)
      ...
      for k = 1:prod(q2.size); 
         temp = d @OPERAND@ reshape(q2.data(:,k),q2.shape);
         q.data(:,k) = temp(:); 
      end
   elseif isequal(prod(q2.size),1)
      ...
      for k = 1:prod(q1.size); 
         temp = reshape(q1.data(:,k),q1.shape) @OPERAND@ d;
         q.data(:,k) = temp(:); 
      end
   else   
...
elseif isa(q1,'qo') & isa(q2,'double')
   ...
   if isequal(prod(size(q2)),1)
      q = q1; q.data = q.data @OPERAND@ sparse(q2);
   elseif isequal(q1.size,size(q2));
      q = q1; for k = 1:prod(q1.size); q.data(:,k) = q.data(:,k) @OPERAND@ sparse(q2(k)); end   
   else
...
elseif isa(q2,'qo') & isa(q1,'double')
   ...
   if isequal(prod(size(q1)),1)
      q = q2; q.data = sparse(q1) @OPERAND@ q.data;
   elseif isequal(q2.size,size(q1));
      q = q2; for k = 1:prod(q2.size); q.data(:,k) = sparse(q1(k)) @OPERAND@ q.data(:,k); end   
   else
      error('Incompatible quantum object and double matrix.');
   end   
...
def generate1(fname,opname,operand,comment):
