...
% QO/MRDIVIDE performs matrix right division on quantum array objects
...
if isa(q1,'qo') & isa(q2,'qo')
   ...
   elseif isequal(q1.dims{2},q2.dims) % Super-operator and flattened operator
      error('Cannot right divide superoperator by an operator');
...
elseif isa(q2,'qo') & isa(q1,'double')   
   ...
   if isequal(prod(size(q1)),1)
      q = struct(q2.qobase);
      for k = 1:prod(q.size)
         temp = q1 / ( reshape(q.data(:,k),q.shape));
         q.data(:,k) = temp(:);
      end
   elseif isequal(size(q2),size(q1))
      q = struct(q2.qobase);
      for k = 1:prod(q.size) 
         temp = q1(k) / ( reshape(q.data(:,k),q.shape)); 
         q.data(:,k) = temp(:);
      end   
