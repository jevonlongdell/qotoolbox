function nexpectations=mccorr2file(fname,rhs,collapses,expectations,ic,corr,tlist,ntraj,options)
...
fid = fopen(fname,'wb');
nR = nterms(rhs);
de2file5(fid,N,nR,ncoll,nexpect);
for k = 1:nR
   params = rhs{k}.params;
...
options.seed
de2file6(fid,ic,corr,tlist,options.seed,ntraj(1),ntraj(2));

if ~isfield(options,'lmm'), options.lmm = 'ADAMS'; end
if ~isfield(options,'iter'), options.iter = 'FUNCTIONAL'; end
if ~isfield(options,'reltol'), options.reltol = 1e-6; end
if ~isfield(options,'abstol'), options.abstol = 1e-6; end
deoptions(fid,options.lmm,options.iter,options.reltol,options.abstol);
fclose(fid);
