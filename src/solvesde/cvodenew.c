 ...
 * Modified 18 December 1998 to allow restarting of integrator    *
 *  without re-allocation of memory: Sze M. Tan			  *
 *----------------------------------------------------------------*
...
#define RDIV      TWO           /* declare divergence if ratio del/delp >
                                 * RDIV  */
#define MSBP       20           /* max no. of steps between lsetup calls        */

#define TRY_AGAIN  99           /* control constant for CVnlsNewton - should
                                 * be */
...
#define MSG_Y0_WRONGSIZE CVM "y0 has wrong size.\n\n"
                            ...
                            void *machEnv)
{
    ...
    cv_mem->cv_N = N;           /* readability constants defined below
                                 * CVodeMalloc */
    cv_mem->cv_f = f;
...
/* Reuse a cvode_mem structure, but with a new initial condition. */
/*  SMT 6-Apr-99 */
/******************** CVodeRestart *****************************/
int     CVodeRestart(void *cvode_mem, real t0, N_Vector y0)
{
    CVodeMem cv_mem;
    ...
    if (N_VLENGTH(y0) != N) {
        fprintf(errfp, MSG_Y0_WRONGSIZE);
        return (ILL_INPUT);
    }
    tn = t0;
    ...
    *t = tn;
    ...
    if (nst == 0) {
        ...
        if (h == ZERO) {
            ...
            if (!hOK) {
/* Ignore error for small step sizes, i.e., just do nothing. SMT 6-Apr-99 */
...
int     CVodeDky(void *cvode_mem, real t, int k, N_Vector dky)
{
    ...
    /* Take absolute value of tfuzz so negative times work. SMT 6-Apr-99 */
    if (tfuzz < 0) tfuzz = -tfuzz;
    tp = tn - hu - tfuzz;
    tn1 = tn + tfuzz;
    if ((t - tp) * (t - tn1) > ZERO) {
...
static bool CVHin(CVodeMem cv_mem, real tout)
{
    ...
    /* Set lower and upper bounds on h0, and take geometric mean Exit with
     * this value if the bounds cross each other       */
     ...
     * differ by a factor < 2. Stop if hnew/hg > 2 after one iteration, as
     * this probably means that the ydd value is bad because of cancellation
     * error.        */
...
static void CVAdjustAdams(CVodeMem cv_mem, int deltaq)
{
     ...
     * The coefficients in the adjustment are the coefficients of the
     * polynomial x*x*(x+xi_1)*...*(x+xi_j), integrated, where xi_j = [t_n -
     * t_(n-j)]/h.               */
...
static int CVnlsNewton(CVodeMem cv_mem, int nflag)
{
    ...
    /* Looping point for the solution of the nonlinear system. Evaluate f at
     * the predicted y, call lsetup if indicated, and call CVNewtonIteration
