echo off
% XADIAB4 uses the quantum state diffusion algorithm to solve for problem
%  of adiabatic transfer of Zeeman coherence of a multi-level atom 
%  to a cavity
...
CdC = C'*C;
%--------------------------------------------------------------------------
Heff = H1 - 0.5*i*sum(CdC);
Heff = Heff + 50*H2*fn('gauss',2.3,1/sqrt(8*log(2))) + 20*H3*fn('gauss',1.7,1/sqrt(8*log(2)));
...
ntraj = 50;
%--------------------------------------------------------------------------
% Write out the file for quantum Monte Carlo solution
%--------------------------------------------------------------------------
nexpect = sde2file('adiab4i.dat',-i*Heff,{C{1},C{2},C{3},C{4}},{},{a'*a,(a'*a)^2},psi0,tlist,ntraj);
%--------------------------------------------------------------------------
% Call the equation solver
%--------------------------------------------------------------------------
sdesolve('adiab4i.dat','adiab4o.dat');
%--------------------------------------------------------------------------
pause; % Press [Enter] to read in output file
%--------------------------------------------------------------------------
fid = fopen('adiab4o.dat','rb');
[iter,nphot,n2phot] = expread(fid,nexpect,tlist);
fclose(fid);
Qparam = (n2phot-nphot.^2)./nphot - 1;
