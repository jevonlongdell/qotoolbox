...
*/
    ...
    /* Set r_norm = beta to L2 norm of V[0] = sb P1_inv r_0, and return if
     * small  */
    ...
    *res_norm = r_norm = beta = RSqrt(N_VDotProd(V[0], V[0]));
    ...
    for (ntries = 0; ntries <= max_restarts; ntries++) {
        ...
        for (l = 0; l < l_max; l++) {
            ...
            /* Generate A-tilde V[l], where A-tilde = sb P1_inv A P2_inv
             * sx_inv */
            ...
            /* Apply left preconditioning: vtemp = P1_inv A P2_inv sx_inv
             * V[l] */
            if (preOnLeft) {
            ...
            /* Update residual norm estimate; break if convergence test
             * passes */
        ...
        if (converged) {
            ...
            /* Add correction to initial x to get final solution x, and
             * return */
    ...
    /* Failed to converge, even after allowed restarts. If the residual norm
     * was reduced below its initial value, compute and return x anyway.
